...
package org.paumard.solutions.codeone2019.E_Collectors;
...
public class O_SimpleCollectors {
    ...
    public void n_simpleCollector03() {
        ...
        int minLength = input.stream().map(String::length).collect(Collectors.minBy(Comparator.naturalOrder())).get();

        // You can also write it in that way with a single collector:
        minLength = input.stream().collect(
                Collectors.mapping(String::length, Collectors.minBy(Comparator.naturalOrder()))).get();
    ...
    public void n_simpleCollector05() {
        ...
        int sum = input.stream().collect(Collectors.summingInt(number -> number));
