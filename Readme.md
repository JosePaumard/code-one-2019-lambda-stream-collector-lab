...
This is the Java 8 branch for Eclipse. Use it with a version 8 of the JDK and the Eclipse IDE. 

You can check the Preferences of Eclipse for the `Folding` property, in `Java | Editor | Folding`, and make sure the `Comments` checkbox is checked (it is not by default). With this the hints we put with the exercises will be closed by default when you open an exercise file. 
